...
Source conflict order is explicit: the user's latest factual correction takes precedence over the Notion page, the Notion page takes precedence over local fallback snapshots, and local snapshots are used only when Notion is unavailable. More specific rules later in this skill (for example, the user-requested language entry, military-service display, or IT/AI education entries) override a general JD-filtering rule, but never override the evidence gate.
...
- Recompute the one-line headline for every JD. Start from the JD's highest-priority requirement, then connect it to a verified cross-project differentiator; never reuse one fixed headline across applications. Avoid generic breadth claims or abstract outcomes such as bare `다양한 화면` or `AX에 기여`. For a frontend JD, a candidate pattern is `React·Next.js 기반으로 다양한 사용자 흐름을 화면·API·상태로 구현해 온 개발자`; for an AI/AX JD, foreground verified Agent and work-flow implementation instead.
