...
- Submitted resumes should include separate `지원동기` and `소개` sections unless the company's required form prevents that structure. `지원동기` explains why the company/JD/role matches the candidate's verified experience; `소개` shows who the candidate is and how they work. Do not repeat the same sentence or evidence in both sections.
