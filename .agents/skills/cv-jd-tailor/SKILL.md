...
- For tailored portfolios, choose experience only from DGM UnitOne, Krafton Jungle, and SKALA by default. Prioritize DGM UnitOne. Use Krafton Jungle Code Sync or SKALA assignments only when DGM UnitOne lacks direct evidence for the JD's qualification/preferred-qualification signals. Do not use Chosun University research, Artificial Society, Boostcourse, Elice, or Smilegate Winter Dev as representative portfolio cases unless the user explicitly asks.
...
- For frontend/product roles, prefer UI/state/API/data-flow labels such as `실시간 협업 상태 처리`, `업무 화면·API 구현`, `검색·하이라이트·메시지 이동 UX`, and `프로젝트 분리 영향도 분석`. Reduce AI/AWS/database keywords on page one when they do not directly support the JD.
- Build core-competency bullets by this sequence: extract the JD's qualification/preferred-qualification wording, find direct source facts in Notion/local source/user latest corrections, choose matching phrasing from the user's approved work-expression candidates, then rewrite into one natural sentence that shows problem/constraint, candidate-owned judgment or implementation, and result. Do not use a phrase that is neither in the JD nor supported by source facts.
...
- For FBITI, use these as candidates for frontend/product/business-platform/SaaS JDs: `수발주·영업관리·계정관리 기능이 결합된 구조에서 기능 간 의존성을 직접 분석`, `36K LOC·300+ 파일·39개 API`, `본사/브랜드 식별자 의존성 제거·치환 범위`, `수발주·영업관리·계정관리 영역을 별도 프로젝트 구조로 나누는 기준`, `고객용 CRM·마케팅 플랫폼 리뉴얼`, `고객 프로모션 기능`, `가맹점주 댓글 응답 자동화`, and `반응형 UI 및 크로스브라우징 대응`.
...
- For CPPM/AX positioning, prefer the user's safe phrases: `Bedrock Agent, SQS, WebSocket 진행 상태 UI를 연결` and `외부 LLM 사용이 제한된 환경에서 AI 기능을 업무 화면으로 구현`. In self-introductions/headlines, lead with security constraints, work-platform context, AI Agent functionality, and screen/API/asynchronous-flow integration instead of client-name signaling.
...
- For 프비티 decomposition work, do not write `service separation`, `independent service`, `standalone sales/operation`, `standalone execution`, or `233 dependency sites fully removed`. Use the user's latest correction: the candidate analyzed 36K LOC, 300+ files, 39 APIs, and headquarters/brand identifier dependency removal/replacement scope to split the strongly coupled order/procurement, sales-management, and account-management areas into separate project structures and organize the account-management flows needed in each project. In external submitted artifacts, do not write internal code names or internal identifier names alone, such as `cvve-tenant`, `cvve-sales`, `hqID`, or `brandID`; write reviewer-readable Korean such as `본사/브랜드 식별자` and `수발주·영업관리·계정관리 영역`.
...
- Do not include Naver Boostcourse/Boostcamp Coaching Study in submitted resumes by default for any JD. Unless the user explicitly asks for it for a specific application, remove sentences such as `Python·데이터·AI 학습은 네이버 부스트코스/부스트캠프 코칭스터디로 이어졌습니다`.
...
- One-line headlines should expose a verified differentiator, not generic self-positioning. For AX/AI/FDE roles, prefer natural submitted-document wording like `보안 제약이 있는 업무 플랫폼에서 AI Agent 기능을 화면·API·비동기 처리까지 구현한 개발자`, grounded in verified enterprise work-platform experience, multi-agent AI functionality, response/progress UI, API, and asynchronous-flow integration. Avoid awkward internal review wording such as `현업 AI 서비스`, `현업 AI Agent 기능`, or `현업 업무 플랫폼`. In sensitive client contexts, omit the client name in headlines, self-introductions, and motivations while preserving the precise project description inside the relevant project section.
