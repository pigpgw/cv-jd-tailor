...
- For 프비티 decomposition work, do not write `service separation`, `independent service`, `standalone sales/operation`, `standalone execution`, or `233 dependency sites fully removed`. Use the user's latest correction: the candidate analyzed 36K LOC, 300+ files, 39 APIs, and hqID/brandID dependency removal/replacement scope to split the strongly coupled order/procurement, sales-management, and account-management areas into separate cvve-tenant and cvve-sales projects and organize the account-management flows needed in each project.
...
- One-line headlines should expose a verified differentiator, not generic self-positioning. For AX/AI/FDE roles, prefer positioning like `대기업 업무 플랫폼의 AX 전환 기능을 화면·API·다중 Agent 흐름까지 설계·개발해 본 개발자`, grounded in verified enterprise service work, multi-agent AI functionality, response/progress UI, API, and asynchronous-flow integration. In sensitive client contexts, omit the client name in headlines, self-introductions, and motivations while preserving the precise project description inside the relevant project section.
